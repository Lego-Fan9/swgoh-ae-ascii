...
var chars = ' .,:;i1tfLCG08@', num_c = chars.length - 1;
export function setChars(input) {
    chars = input;
    num_c = chars.length - 1;
}
export function asciify(path, second, third) {
    // Organize arguments
    var opts = {}, callback;
...
var asciify_core = function (path, opts, callback) {
    ...
    Jimp.read(path, function (err, image) {
        if (err)
            return callback('Error loading image: ' + err);
...
var calculate_dims = function (img, opts) {
    switch (opts.fit) {
        ...
        case 'box':
            var w_ratio = img.bitmap.width / opts.width, h_ratio = img.bitmap.height / opts.height, neww, newh;
            if (w_ratio > h_ratio) {
...
//# sourceMappingURL=asciify-image.js.map